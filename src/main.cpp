...
int64_t devCoin = 2*COIN/10; //1% per block developer coin reward
...
const string strMessageMagic = "CKcoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    {
        nSubsidy = 600000 * COIN; // 2% Bittrex Swap Block for all users of CKcoins (HASH, DRM, NEB, TECH, FIRE, and ISIS) and all active Bittrex members
    }
		else if(pindexBest->nHeight < 250)
    {
		nSubsidy = 1 * COIN; // low CK reward to prevent instamine
    }
		else if(pindexBest->nHeight < 1500000)
    {
		nSubsidy = 20 * COIN; // long pow phase of 2.8 years
    }
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
	...
	if(IsProofOfWork()) //Check developer coinbase reward
    {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "CKcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "CKCOIN CK It is finally here 20140508";
        CTransaction txNew;
        txNew.nTime = 1407280150;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1407280150;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2154497;
		if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x15735b56b3c3a74bfa5ceb53eea08c1eba93c1e44b86976bb7de7daf2ea955d9"));
        block.print();
